...
{
    /// <summary>
    /// What the Screen Scanner does when a cell is edited. The scan that fills the panel is covered
    /// by ScreenScannerRunnerTests; this is the other half - the write back to the Root entry, the undo
    ...
    /// </summary>
    public class ScreenScannerEditsTests
    {
        ...
        private ScreenScannerWindow _window;
        private RootBase _root;

        [SetUp]
        public void SetUp()
        {
            _scan = new ScreenScannerRunner(new ScreenSubContextDeclarations());
            _window = ScriptableObject.CreateInstance<ScreenScannerWindow>();
        ...
        {
            MethodInfo method = typeof(ScreenScannerWindow).GetMethod(name, BindingFlags.Instance | BindingFlags.NonPublic);
