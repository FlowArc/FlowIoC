...
namespace FlowIoC.Tests
{
    public class ModulePageTests
    {
        ...
        /// </summary>
        private class SparsePage : ModulePage
        {
        ...
        private class DemandingPage : ModulePage
        {
        ...
        /// <summary>
        /// The band under the banner falls back to the title, and the install dialog to its
        /// generic sentence, so a page that has nothing to add to either says nothing.
        /// </summary>
        [Test]
        public void A_page_names_no_headline_no_tagline_and_no_hint_unless_it_says_otherwise()
        {
            Assert.IsNull(new SparsePage().BodyHeadline);
            Assert.IsNull(new SparsePage().BodyTagline);
            Assert.AreEqual(string.Empty, new SparsePage().InstalledHint);
        }
        ...
        {
            Assert.IsTrue(typeof(ModulePage).IsPublic, "ModulePage is not public.");
