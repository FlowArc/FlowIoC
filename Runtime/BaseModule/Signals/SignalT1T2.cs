...
namespace FlowIoC.BaseModule.Signals
{
    public class Signal<T1, T2> : SignalBody, ISignal<T1, T2>
    {
        ...
        /// </summary>
        public void RemoveListenerOnce(Action<T1, T2> listener)
        {
            _callbackOnce -= listener;
        }
        ...
        {
            if (!_hideCommandLog)
                FlowLogger.LogDispatch(_isFrameworkOwned, "Signal is dispatched: '", _name, "' with 2 parameters!");
            ...
            Action<T1, T2> once = _callbackOnce;
            _callbackOnce = null;
            once?.Invoke(param1, param2);
    ...
    public interface ISignal<T1, T2> : ISignalBody
    {
        ...
        void RemoveListenerOnce(Action<T1, T2> listener);
