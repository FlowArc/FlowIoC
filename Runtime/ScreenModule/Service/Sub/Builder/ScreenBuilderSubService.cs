...
namespace FlowIoC.ScreenModule.Service.Sub.Builder
{
    ...
    public class ScreenBuilderSubService : IScreenBuilderSubService
    {
        ...
        {
            if (_screenRuntimeModel.GetScreen<T>(managerId, out T pooled))
            {
                ScreenVO pooledData = pooled.Data;
                pooledData.ManagerId = managerId;
                _registry.CopyDataFromConfig(pooledData);

                FlowLogger.Log(SystemLogType.Screen, $"[ScreenService.Open] {pooledData.ScreenType.Name}");
                return new ScreenBuilder(pooledData, pooled, false, _screenRuntimeModel, _show, _hide);
            }
            ...
            if (!_registry.TryGetEntry(managerId, typeof(T), out ScreenEntry entry))
            {
                ...
                return new ScreenBuilder(null, null, true, _screenRuntimeModel, _show, _hide);
            }
            ...
            FlowLogger.Log(SystemLogType.Screen, $"[ScreenService.Open] {screenData.ScreenType.Name}");
            return new ScreenBuilder(screenData, null, false, _screenRuntimeModel, _show, _hide);
        }
    }
}