...
| `Tools/FlowIoC/Module Configuration/Update Namespace Settings` | Rewrites every `.csproj.DotSettings` and the solution file from the modules that exist, and deletes orphaned ones |
...
in the code generator settings, run `Update Namespace Settings` rather than editing them - it
