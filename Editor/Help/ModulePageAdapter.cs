...
namespace FlowIoC.Editor.Help
{
    ...
    /// in. Written once here rather than once per module, which is the whole reason a page can
    /// get away with declaring so little - FlowIoC's own pages included.
    /// </summary>
    internal class ModulePageAdapter : HelpPage
    {
        ...
        private ModuleInstallState State() =>
            new ModuleInstallState(_payload.IsResolved, IsInstalled(), AbsentAssemblies());
        ...
        /// often enough that doing the walk each time would cost real frames.
        /// </summary>
