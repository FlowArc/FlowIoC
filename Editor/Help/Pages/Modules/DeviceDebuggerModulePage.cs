    ...
    internal class DeviceDebuggerModulePage : ModulePage
    {
        ...
        private void DrawTabs(HelpPainter painter)
        {
            ...
            painter.Paragraph(
                ...
                + "as a sheet over the lower third, with the time, the source and the stack trace, its own "
                + "Copy for that one row, and Close; Copy all logs beside Clear puts the whole ring on the "
                + "clipboard, which is the one export a phone has. The panel's own traffic is logged too "
                + "- a tap on an option reads as its dispatch under it.");
